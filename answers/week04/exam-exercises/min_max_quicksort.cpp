...
void run_benchmark() {
  ...
  const int64_t REPEAT = 3;
  std::vector<int64_t> speedup_custom(MAX_THREADS);
  std::vector<int64_t> speedup_gnu(MAX_THREADS);
  std::vector<int64_t> threads(MAX_THREADS);
  ...
  for (int num_threads = 1; num_threads <= MAX_THREADS; num_threads++) {
    ...
    for (int i = 0; i < REPEAT; i++) {
      ...
      std::vector<int64_t> data_copy =
          data;  // duplicate data for min_max_quicksort
      std::vector<int64_t> data_copy_parallel =
          data;  // duplicate data for __gnu_parallel::sort
      ...
      // std::cout << "speedup over std::sort: "
      //           << time_std_sort / time_min_max_quicksort << "\n\n";
    ...
    speedup_custom[num_threads - 1] /= REPEAT;
    speedup_gnu[num_threads - 1] /= REPEAT;
  }

  plt::plot(threads, speedup_custom,
    {{"label", "min_max_quicksort"}, {"color", "blue"}});
  plt::plot(threads, speedup_gnu,
    {{"label", "gnu_parallel"}, {"color", "red"}});
  plt::xlabel("Number of threads", {{"fontsize", "12"}});
  plt::ylabel("Relative speedup against std::sort", {{"fontsize", "12"}});
  plt::title("Time seedup against std::sort", {{"fontsize", "16"}, {"fontweight", "bold"}});
  std::vector<int> xticks = {1, 5, 10, 15, 20};
  plt::xticks(xticks);
  std::vector<int> yticks = {2, 4, 6, 8, 10};
  plt::yticks(yticks);
